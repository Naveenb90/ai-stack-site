...
A reference map of public (and a few private) companies across the AI supply chain, plus a weekly stock-price refresh pipeline and a monthly SEC-filings refresh pipeline. See `CLAUDE.md` for full project notes (layer model, data conventions, known limitations).
...
- `sec_filings.json` — shared SEC filing data (CIK, latest annual report date + link per ticker), machine-owned once its Action has run.
- `scripts/update_prices.py` — pulls weekly data from Yahoo Finance and rewrites `prices.json`.
- `scripts/fetch_sec_filings.py` — pulls each ticker's latest annual report from SEC EDGAR and rewrites `sec_filings.json`. Imports its ticker list straight from `update_prices.py` rather than keeping a second copy.
...
- `.github/workflows/update-sec-filings.yml` — runs the SEC filings script monthly (1st of the month, 06:00 UTC), and can be triggered manually from the Actions tab.
...
No API keys or secrets are needed — both pipelines run entirely off free, keyless endpoints (Yahoo Finance for prices, SEC EDGAR for filings).
...
A second, independent pipeline was added for SEC filing info: `scripts/fetch_sec_filings.py` looks up each ticker's CIK via SEC's bulk `company_tickers.json`, then reads its filing history from SEC EDGAR's `submissions` API to find the most recent 10-K (or 20-F/40-F for foreign private issuers like TSMC, ASML, Arm, SAP, Nebius), and writes `sec_filings.json`. It runs monthly rather than weekly, since annual reports don't need weekly checking. `sec_filings.json` starts as an empty seed file — the modal falls back to a generic "All filings ↗" EDGAR link (built straight from the ticker, no data file needed) until the workflow's first successful run populates the "latest filing" line for each company.
