...
│ (no price fetch —     │        │ (Yahoo Finance primary, │
│  name/ticker/role only)│       │  Alpaca fallback)       │
└─────────────────────┘        └──────────────────────┘
```
...
## UI: ticker-only chips, click for detail

Chips on `index.html` show only the company name, role, and ticker symbol — no inline price or % badges. Clicking (or Enter/Space on a focused chip) opens a modal populated entirely from data already fetched into `PRICES_DATA`: current price, a 60-session sparkline, 52-week high/low with % from current, YTD %, last-20-days %, and a range slider positioning the price between the 52-week low and high. No network call happens at click time — everything needed is already in `prices.json`, which keeps the page fully static and avoids exposing any API credentials or hitting CORS issues client-side.

For a chip with `data-market="private"` or `data-market="non-us"`, or a US ticker with no entry yet in `prices.json`, the modal shows an explanatory empty state instead of stats.
...
- `_meta.sources` reports how many tickers came from each provider on that run — worth a glance after a run to see whether the Alpaca fallback got used unexpectedly often (a sign Yahoo's endpoint is having a bad day).
...
| **Pending** | `data-ticker="SYM"`, ticker not yet in `prices.json` | Modal shows "no data yet" until the next run picks it up (must also be in the `TICKERS` list in the script) |
| **Permanently no data** | `data-market="non-us"` or `data-market="private"` | Modal shows a fixed explanatory message; explicitly skipped by the pipeline forever — foreign-listed (Samsung, SK Hynix, Kioxia, Neo Performance Materials) or private (Anthropic, OpenAI, xAI, Mistral AI, Perplexity) |
...
- **Yahoo's chart endpoint is unofficial/undocumented.** It's free and reliable in practice, but Google/Yahoo can change its shape or start blocking without notice — that's exactly why the Alpaca fallback exists. See [data-flow.md](./data-flow.md).
